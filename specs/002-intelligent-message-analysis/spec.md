...
- System must analyze messages within 5 seconds of synchronization
...
**Priority**: High
**Description**: Display analyzed messages with visual indicators

- Dashboard must show message list with analysis badges:
  - Spam messages: Red "SPAM" badge
  - Response required: Orange "Needs Reply" badge
  - No action needed: Green "Reviewed" badge
- Badges must display before message subject line
- Messages must be sortable by analysis category
- Messages must be filterable by spam/needs-reply/reviewed status
- Clicking message must display full analysis details

### FR-006: AI Reply Suggestion Interface
**Priority**: High
...
### FR-007: Auto-Delete Spam Setting
**Priority**: Medium
**Description**: Configurable automatic spam deletion

- Settings page must include "Auto-delete spam" toggle in Message Management section
- When enabled:
  - Messages with spam probability >80% are automatically moved to Trash
  - Action occurs within 10 seconds of spam detection
  - User receives daily summary email with deleted spam count
- Deleted spam must remain in Trash for 30 days
- Setting must persist across user sessions

### FR-008: Auto-Send Reply Setting
**Priority**: Medium
**Description**: Configurable automatic reply sending

- Settings page must include "Auto-send replies" toggle in Message Management section
...
- Users must be able to configure conditions:
  - Specific sender whitelist/blacklist
  - Business hours only option
  - Maximum auto-replies per day limit
- Setting must persist across user sessions
